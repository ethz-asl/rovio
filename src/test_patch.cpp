...
// Test isPatchInFrame
TEST_F(PatchTesting, isWarpedPatchInFrame) {
  Eigen::Matrix2f aff;
  aff.setIdentity();
  c_ = cv::Point2f(0,0);
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff),false);
  c_ = cv::Point2f(patchSize_/2-0.1,patchSize_/2-0.1);
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff),false);
  c_ = cv::Point2f(patchSize_/2,patchSize_/2);
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff),true);
  c_ = cv::Point2f(patchSize_/2+0.1,patchSize_/2+0.1);
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff),true);
  c_ = cv::Point2f(imgSize_-patchSize_/2-0.1,imgSize_-patchSize_/2-0.1);
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff),true);
  ...
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff),false);
  c_ = cv::Point2f(0,0);
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff,true),false);
  c_ = cv::Point2f(patchSize_/2+1-0.1,patchSize_/2+1-0.1);
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff,true),false);
  c_ = cv::Point2f(patchSize_/2+1,patchSize_/2+1);
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff,true),true);
  c_ = cv::Point2f(patchSize_/2+1+0.1,patchSize_/2+1+0.1);
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff,true),true);
  c_ = cv::Point2f(imgSize_-patchSize_/2-1-0.1,imgSize_-patchSize_/2-1-0.1);
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff,true),true);
  ...
  c_ = cv::Point2f((sin(M_PI/6.0)+cos(M_PI/6.0))*patchSize_/2+1e-6,(sin(M_PI/6.0)+cos(M_PI/6.0))*patchSize_/2+1e-6);
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff),true);
  c_ = cv::Point2f((sin(M_PI/6.0)+cos(M_PI/6.0))*patchSize_/2+1e-6,(sin(M_PI/6.0)+cos(M_PI/6.0))*patchSize_/2-1e-6);
  ASSERT_EQ(isWarpedPatchInFrame(p_,img1_,c_,aff),false);
  c_ = cv::Point2f((sin(M_PI/6.0)+cos(M_PI/6.0))*patchSize_/2-1e-6,(sin(M_PI/6.0)+cos(M_PI/6.0))*patchSize_/2+1e-6);
...
TEST_F(PatchTesting, extractPatchFromImage) {
  ...
  for(unsigned int n=0;n<N;n++){
    ...
    extractPatchFromImage(p_,img1_,c_);
...
TEST_F(PatchTesting, extractWarpedPatchFromImage) {
  Eigen::Matrix2f aff;
  aff << cos(M_PI/6.0), -sin(M_PI/6.0), sin(M_PI/6.0), cos(M_PI/6.0);
  c_ = cv::Point2f(imgSize_/2,imgSize_/2);
  extractWarpedPatchFromImage(p_,img1_,c_,aff);
  float* patch_ptr = p_.patchWithBorder_;
  for(int i=0;i<patchSize_+2;i++){
    for(int j=0;j<patchSize_+2;j++, ++patch_ptr){
...
TEST_F(PatchTesting, computeGradientParameters) {
  ...
  c_ = cv::Point2f(imgSize_/2,imgSize_/2);
  extractPatchFromImage(p_,img2_,c_);
  p_.computeGradientParameters();
